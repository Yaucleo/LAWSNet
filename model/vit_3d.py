...
class AbsPositionalEncoding1D(nn.Module):
    ...
    def forward(self, x):
        batch = x.size()[0]
        b = repeat(self.abs_pos_enc, 'b ... -> (b tile) ...', tile=batch // self.abs_pos_enc.shape[0])
...
class ViT(nn.Module):
    def __init__(self, img_dim, in_channels, embedding_dim, head_num, mlp_dim,
        ...
        self.n_patches = int((img_dim[0] * img_dim[1] * img_dim[2]) / (patch_dim ** 3))

        self.patch_embeddings = nn.Conv3d(in_channels, embedding_dim,
                                          kernel_size=patch_dim, stride=patch_dim, bias=False)
...
if __name__ == '__main__':
    ...
    print(vit(torch.rand(1, 16, 144, 144, 20)).shape)
